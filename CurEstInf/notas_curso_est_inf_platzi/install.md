# Notas_Curso_Est_Inf_Platzi guide installation
...
activate notas_curso_est_inf_platzi
```
...
To move beyond notebook prototyping, all reusable code should go into the `notas_curso_est_inf_platzi/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `notas_curso_est_inf_platzi` folder and use the modules inside your notebooks :
...
```python
from notas_curso_est_inf_platzi.utils.paths import data_dir
data_dir()
```
